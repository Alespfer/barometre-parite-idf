...
knitr::opts_chunk$set(eval = FALSE, echo = TRUE, message = TRUE, warning = TRUE)
```
...
```{r}
# Étape 2.4 (NOUVEAU) : Harmonisation de l'indicateur d'augmentation
# L'indicateur "note_augmentation" est sur 20 pts (>250 sal.) ou 35 pts (50-250 sal.)
# Nous le normalisons sur une base commune de 20 points pour permettre des moyennes cohérentes.
message("--- Harmonisation de l'indicateur d'augmentation ---")
egapro_prepared <- egapro_prepared %>%
  dplyr::mutate(
    note_augmentation_harmonisee = dplyr::case_when(
      # Pour les PME, on ramène le score de 35 à 20
      tranche_effectifs == "50 à 250" ~ (note_augmentation / 35) * 20,
      # Pour les autres, le score est déjà sur 20
      TRUE ~ note_augmentation
    )
  )

message("✅ Colonne 'note_augmentation_harmonisee' créée.")
```
...
# PREMIÈRE PINCE : On charge la carte V2 complète, avec le "brouillard de guerre" MGP
# Appel de la fonction unique et blindée
# Appel de la fonction unique et blindée
...
# Étape A : Créer les features socio-démographiques
ze_socio_features <- aggregate_socio_to_ze(communes_features)

# Étape B : Dédoublonner les données SIRENE
sirene_deduplicated <- sirene_clean %>%
  dplyr::distinct(siren, .keep_all = TRUE)

# Étape C (NOUVEAU) : Décontamination de la carte de référence
map_deduplicated <- map_com_prepared %>%
  dplyr::distinct(com_code, .keep_all = TRUE)

# Étape D : Construction de la table finale
master_df_historique <- egapro_prepared %>%
  ...
  inner_join(
    ...
    relationship = "many-to-one" # On impose la discipline : plusieurs entreprises PAR commune
  ) %>%
  left_join(
    ze_socio_features %>% select(-ze_name),
    by = "ze_code"
  )
