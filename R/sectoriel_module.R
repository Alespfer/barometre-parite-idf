...
# dédié à la comparaison des performances Egapro entre les secteurs d'activité.
# ==============================================================================
...
sectoriel_ui <- function(id, df) {
  ...
  shiny::fluidRow(
            ...
            bslib::accordion_panel(
              ...
              shiny::div(class = "d-flex align-items-center",
                         ...
                         bslib::tooltip(
                           ...
                           "L'Index est obligatoire pour les entreprises de 50 salariés et plus.",
                           placement = "right"
                         )
              ),
              # Ce filtre permet à l'utilisateur de choisir des secteurs spécifiques.
              ...
              shiny::tags$small(
                class = "text-muted",
                shiny::icon("info-circle"), 
                "Par défaut, l'affichage montre les 5 meilleurs et 5 moins bons secteurs."
              ),
    ...
    # --- Colonne de droite : Visualisations (KPIs, Graphique, Tableau) ---
    shiny::column(
      width = 9,
      ...
      bslib::card(
        bslib::card_header(
          shiny::div(class = "d-flex justify-content-between align-items-center",
                     ...
                     # Widget pour donner la possibilité de basculer vers une palette de couleurs accessible.
        ...
        bslib::card_footer(
          ...
          shiny::div(
            ...
            "Cliquez sur un secteur dans le graphique pour l'analyser sur la carte."
          )
        )
      ),
      bslib::card(
...
sectoriel_server <- function(id, master_df_historique, palette_accessible, shared_state) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # --- Logique réactive (préparation des données) ---
    
    
    # Filtre les données en fonction de tous les contrôles de l'UI. 
    data_secteur_filtree <- shiny::reactive({
      ...
      # Logique d'affichage : soit tous les secteurs, soit une sélection, soit le top/bottom 5 par défaut.
      ...
      # Logique par défaut : on calcule la médiane par secteur et on prend les 5 meilleurs et 5 moins bons (>= 10 occurrences)
      sector_summary <- df %>%
    ...
    # Affiche les KPIs au-dessus du graphique.
    ...
    output$plot_secteur <- plotly::renderPlotly({
      ...
      # Gère la bascule entre la palette de couleurs par défaut et la palette accessible.
      ...
      # Calcule dynamiquement la hauteur et la marge du graphique. 
    ...
    # Cet observateur se déclenche lorsque l'utilisateur clique sur le graphique Plotly.
    shiny::observeEvent(plotly::event_data("plotly_click", source = "plot_secteur_source"), {
      ...
      # On met à jour l'état partagé (`shared_state`) pour communiquer avec le module carte.
      shared_state$selected_sector <- clicked_sector
      shared_state$trigger_zoom <- TRUE  
      
      # On affiche une notification pour informer l'utilisateur.
      notification_ui <- shiny::div(
        "Filtre appliqué sur le secteur : ", shiny::strong(clicked_sector), ". ",
        shiny::tags$a(
          href = "#",
          onclick = "document.querySelector('a[data-value=\"Carte & Territoires\"]').click(); return false;",
